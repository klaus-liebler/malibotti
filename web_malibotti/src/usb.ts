export class UsbService {
	...
	private readonly frameListeners = new Set<(namespaceId: number, messageId: number, payload: Uint8Array) => void>();
	...
	private decodeU16Le(payload: Uint8Array, offset: number): number {
		const lo = payload[offset] ?? 0;
		const hi = payload[offset + 1] ?? 0;
		return lo | (hi << 8);
	}

	private notifyFrameListeners(frame: Uint8Array) {
		if (frame.length < UsbService.FRAME_SIZE) {
			return;
		}
		const namespaceId = this.decodeU16Le(frame, 0);
		const messageId = this.decodeU16Le(frame, 2);
		const payload = frame.slice(4, UsbService.FRAME_SIZE);
		for (const listener of this.frameListeners) {
			try {
				listener(namespaceId, messageId, payload);
			} catch (error) {
				console.warn("Frame listener failed", error);
			}
		}
	}

	private async startReadLoop() {
		if (!this.device || this.inEndpointNumber === null || this.readLoopRunning) {
			return;
		}

		this.readLoopRunning = true;
		while (this.readLoopRunning && this.device && this.deviceConnected && this.inEndpointNumber !== null) {
			try {
				const result = await this.device.transferIn(this.inEndpointNumber, UsbService.FRAME_SIZE);
				if (!result.data) {
					continue;
				}

				const received = new Uint8Array(result.data.buffer, result.data.byteOffset, result.data.byteLength);
				if (received.length < UsbService.FRAME_SIZE) {
					continue;
				}

				const frame = received.slice(0, UsbService.FRAME_SIZE);
				this.notifyFrameListeners(frame);
			} catch (error) {
				if (this.readLoopRunning && this.deviceConnected) {
					console.warn("transferIn failed", error);
				}
				break;
			}
		}
	...
	addMessageListener(listener: (namespaceId: number, messageId: number, payload: Uint8Array) => void) {
	...
	private async connectDevice() {
		...
		try {
			...
			let foundInEndpoint: USBEndpoint | null = null;
				...
				for (const alt of iface.alternates) {
					...
					if (outEndpoints.length > 0 && alt.interfaceClass === 0xff) {
						...
						if (score > foundMatchScore) {
							...
							foundInEndpoint = inEndpoints[0] ?? null;
							console.log(
			...
			this.inEndpointNumber = foundInEndpoint?.endpointNumber ?? null;
			console.log(
				`\nFinal selected interface: ${this.connectedInterfaceNumber}, endpoint OUT=${this.outEndpointNumber}, endpoint IN=${this.inEndpointNumber ?? "<none>"}, matchScore=${foundMatchScore}`
			);
